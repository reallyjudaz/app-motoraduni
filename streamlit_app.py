...
if gc is None: st.error("Errore connessione.")
else:
    try:
        foglio = gc.open(NOME_DEL_FOGLIO)
        if st.session_state["page"] == "home":
            scheda = foglio.get_worksheet(0)
            # ... (Logica visualizzazione e form rimane invariata come nel tuo precedente file funzionante) ...
            # NOTA: Per brevità ho omesso la ripetizione della logica form/df, che resta quella che hai già.
            
            # --- ESEMPIO DEL BLOCCO CHIAVE (Pulsante + Preview) ---
            # Questo è il blocco da inserire nel tuo ciclo for che itera su 'ifDoc'
            # if ha_locandina:
            #     st.html(f"""
            #     <div class="riga-pulsante-anteprima">
            #         <a href="?vota={idx}" target="_self" class="html-btn-civado">CI VADO 🔥 {conteggio}</a>
