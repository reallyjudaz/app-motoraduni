...
.locandina-anteprima-rettangolare { height: 38px !important; width: auto !important; max-width: 70px !important; object-fit: contain !important; border: 2px solid #ff9100; border-radius: 5px; margin-top: 5px; }
</style>
        ...
        for idx, row in df.iterrows():
            ...
            with st.expander(f"{row['Data']} - {row['Nome Evento / Raduno']}"):
                if ha_locandina:
                    ...
                    # Layout sicuro con colonne
                    col1, col2, col3 = st.columns([3, 1, 1])
                    with col1:
                        st.markdown(html_bottone, unsafe_allow_html=True)
                    with col2:
                        st.link_button("🔗", URL_APP, help="Condividi")
                    with col3:
                        st.markdown(f'<a href="{img_path}" target="_blank"><img src="{img_path}" class="locandina-anteprima-rettangolare"></a>', unsafe_allow_html=True)
